...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSPredicate *predicate = [NSPredicate
                              predicateWithFormat:@"projectName == %@ and locationName ==%@ and areaName == %@",
                              _projectName, _locationName, cell.textLabel.text];
    [request setPredicate:predicate];
    
    NSError *error = nil;
    NSArray *objects = [_managedObjectContext executeFetchRequest:request error:&error];
    if(objects == nil) {
        NSLog(@"Error! could not find project, location, area (%@, %@, %@)", _projectName, _locationName, cell.textLabel.text);
    } else {
        if(objects.count >0) {
            Project *project = (Project*)[objects objectAtIndex:0];
            [self.delegate projectDataSelectedFor:project]; 
        }
    }
}
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"areaName" ascending:NO];
    NSArray *sortDescriptors = @[sortDescriptor];
    
    [fetchRequest setSortDescriptors:sortDescriptors];
    
    fetchRequest.resultType = NSDictionaryResultType;
    fetchRequest.propertiesToFetch = [NSArray arrayWithObject:[[entity propertiesByName] objectForKey:@"areaName"]];
    [fetchRequest setReturnsDistinctResults:YES];
    
    NSPredicate *predicate = [NSPredicate
                              predicateWithFormat:@"projectName == %@ and locationName == %@",
                              _projectName, _locationName];
...
- (void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [[object valueForKey:@"areaName"] description];
}
